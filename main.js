function main() {
    ...
    submitButton.addEventListener(`click`, function(){
        ...
        if (userTextInput.value.length > 0) {
            ...
            makeHTMLElement(`p`,`Your submitted text contains ${totalSentences} sentences, ${totalChars} characters, ${data.length} total words and ${uniqueWords.length} unique words. With an average of ${(isFinite(data.length / totalSentences) ? (data.length / totalSentences) : 0).toFixed(2)} words and ${(isNaN(totalPunctuation / totalSentences) ? 0 : (totalPunctuation / totalSentences)).toFixed(2)} punctuation marks per sentence, as well as ${(isNaN(totalChars / data.length) ? 0 : (totalChars / data.length)).toFixed(2)} letters per word; each including ${(isNaN(totalVowels / data.length) ? 0 : (totalVowels / data.length)).toFixed(2)} vowels and ${(isNaN(totalConsonants / data.length) ? 0 : (totalConsonants / data.length)).toFixed(2)} consonants. And took ${totalTime ? totalTime : 0} seconds to be evaluated, at a speed of ${(data.length / (totalTime >= 1 ? totalTime : 1)).toFixed(2)} words per second (WPS).`,`output`);
            makeHTMLElement(`p`,`The estimated reading time for your text is ${(predata.length / 180).toFixed(2)} minutes, while the estimated speaking time is ${(predata.length / 125).toFixed(2)} minutes. Your text also has a novelty score of ${(isNaN((uniqueWords.length / data.length) * 100) ? 0 : ((uniqueWords.length / data.length) * 100)).toFixed(2)}%.`,`secondaryOutput`);
